    ...
    if image_paths:
        ...
        if selected_paths:
            ...
            for path in selected_paths:
                try:
                    img = Image(str(path), width=60 * mm, height=45 * mm)
                except Exception:
                    continue
                row.append(img)
                if len(row) == 3:
        ...
        if image_rows:
            ...
            story.append(
                Paragraph(f"<b>{T('Fotodokumentace', 'Fotodokumentation')}</b>", title_style))
            story.append(Spacer(0, 4 * mm))
...
class StrojeGrid(tk.Tk):
    ...
    def on_resize(self, event):
        if event.widget is self:
            self.after_idle(lambda: self.nakresli_mrizku(None))
