...
def vyber_fotky_dialog(parent, image_paths: list):
    ...
    header.pack(fill="x")
    tk.Label(header, text=T("Vyberte fotky pro export do PDF:", "Wählen Sie Fotos für PDF-Export:"),
             font=("Segoe UI", 11, "bold")).pack(anchor="w")
    tk.Label(header, text=T(f"Celkem nalezeno: {len(image_paths)} fotek • Najeďte myší pro náhled",
    ...
    # Slovník pro uchování checkboxů, jejich stavů a miniatur
    check_vars = []
    check_buttons = []
    thumbnails = []  # Reference na PhotoImage objekty

    # Tooltip okno pro velký náhled
    tooltip_window = None
    tooltip_label = None
    ...
    def _show_preview(event, path):
        # pokud už je otevřený pro stejný soubor, jen posuň okno
        if preview.get("win") is not None and preview.get("path") == str(path):
            try:
                preview["win"].geometry(f"+{event.x_root + 16}+{event.y_root + 16}")
            except Exception:
                pass
            return
        ...
        try:
            ...
            img.thumbnail((700, 520))   # velikost náhledu – uprav dle potřeby
            tk_img = ImageTk.PhotoImage(img)
        except Exception:
            return

        tw = tk.Toplevel(win)  # top = tvůj dialog pro výběr fotek
        tw.overrideredirect(True)
        tw.attributes("-topmost", True)

        lbl = tk.Label(tw, image=tk_img, bd=1, relief="solid", bg="white")
        lbl.pack()

        preview["win"] = tw
        preview["lbl"] = lbl
        preview["img"] = tk_img  # IMPORTANT: držet referenci, jinak se obrázek “ztratí”
        preview["path"] = str(path)

        try:
            tw.geometry(f"+{event.x_root + 16}+{event.y_root + 16}")
        except Exception:
            pass

    def _move_preview(event):
        if preview.get("win") is not None:
            try:
                preview["win"].geometry(f"+{event.x_root + 16}+{event.y_root + 16}")
            except Exception:
                _hide_preview()
# --------------------------------------------------------------------------
    ...
    def show_tooltip(event, image_path):
        """Zobrazí tooltip s velkým náhledem fotky."""
        nonlocal tooltip_window, tooltip_label

        # Zavřít předchozí tooltip
        hide_tooltip()

        try:
            # Načíst obrázek a vytvořit náhled
            img = PILImage.open(image_path)

            # Zachovat poměr stran
            max_size = (700, 520)  # větší náhled
            ...
            # Pozice tooltip (vpravo od kurzoru)
            x = event.x_root + 15
            y = event.y_root - 50

            # Kontrola, aby tooltip nebyl mimo obrazovku
            screen_width = win.winfo_screenwidth()
            screen_height = win.winfo_screenheight()

            # Dočasně umístit pro získání rozměrů
            tooltip_window.geometry(f"+{x}+{y}")

            # Frame s rámečkem
            frame = tk.Frame(tooltip_window, relief="solid",
                             borderwidth=2, bg="white")
            frame.pack()

            tooltip_label = tk.Label(frame, image=photo, bg="white")
            tooltip_label.image = photo  # Uchovat referenci
            tooltip_label.pack(padx=2, pady=2)

            # Info pod obrázkem
            info = tk.Label(frame, text=image_path.name, bg="white",
                            font=("Segoe UI", 8), wraplength=280)
            info.pack(padx=5, pady=(0, 5))

            # Aktualizovat a přepočítat pozici
            tooltip_window.update_idletasks()
            tw = tooltip_window.winfo_width()
            th = tooltip_window.winfo_height()

            # Přepočet X (vlevo/vpravo)
            if x + tw > screen_width - 10:
                x = event.x_root - tw - 15

            # Přepočet Y (dole/nahoře)
    ...
    def hide_tooltip(event=None):
        ...
        # ---- stabilní tooltip (delay + kontrola přechodu myši) ----
    tooltip_after_id = None

    def move_tooltip(event):
        """Posouvá tooltip s myší (když existuje)."""
        nonlocal tooltip_window
    ...
    def delayed_show_tooltip(event, image_path):
        """Zpožděné zobrazení tooltipu – zabrání blikání."""
        nonlocal tooltip_after_id
        cancel_tooltip()
        tooltip_after_id = win.after(
            250, lambda: show_tooltip(event, image_path)
        )

    def cancel_tooltip():
        """Zruší naplánované zobrazení tooltipu."""
        nonlocal tooltip_after_id
        if tooltip_after_id:
            try:
    ...
    # Vytvoření checkboxů s miniaturami
    for i, path in enumerate(image_paths):
        ...
        # Načíst a vytvořit miniaturní náhled (30x30)
        try:
            img = PILImage.open(path)
            img.thumbnail((30, 30), PILImage.Resampling.LANCZOS)
            photo = ImageTk.PhotoImage(img)
            thumbnails.append(photo)  # Uchovat referenci

            # Label s miniaturou
            img_label = tk.Label(frame_item, image=photo, bg="white")
            img_label.pack(side="left", padx=(5, 5))

            # Tooltip na miniatuře
       #     img_label.bind("<Enter>", lambda e, p=path: show_tooltip(e, p))
        #    img_label.bind("<Leave>", hide_tooltip)

            # Tooltip na celý řádek (stabilní – nebliká)
            frame_item.bind("<Enter>", lambda e, p=path: show_tooltip(e, p))
            frame_item.bind("<Motion>", move_tooltip)
            frame_item.bind("<Leave>", hide_tooltip)

            # Ať to funguje i když kurzor je na dětech (miniatura/checkbox)
            for child in frame_item.winfo_children():
                child.bind("<Enter>", lambda e, p=path: show_tooltip(e, p))
                child.bind("<Motion>", move_tooltip)
                child.bind("<Leave>", hide_tooltip)

        except Exception as e:
            # Fallback - prázdný prostor
            spacer = tk.Label(frame_item, text="❌", width=2, bg="white")
            spacer.pack(side="left", padx=(5, 5))

        # Checkbox s názvem souboru
        cb = tk.Checkbutton(frame_item, variable=var, text=f"{i+1}. {path.name}",
                            anchor="w", font=("Segoe UI", 9), bg="white")
        cb.pack(side="left", fill="x", expand=True)
        check_buttons.append(cb)

        # Tooltip i na checkboxu
    #    cb.bind("<Enter>", lambda e, p=path: show_tooltip(e, p))
    ...
    # Tlačítka pro hromadný výběr
    tk.Button(btn_frame, text=T("✓ Vybrat vše", "✓ Alles wählen"),
              command=select_all, width=12).pack(side="left", padx=2)
    tk.Button(btn_frame, text=T("✗ Zrušit vše", "✗ Alles abwählen"),
              command=deselect_all, width=12).pack(side="left", padx=2)
    tk.Button(btn_frame, text=T("⇄ Invertovat", "⇄ Invertieren"),
              command=invert_selection, width=12).pack(side="left", padx=2)

    # Aktualizace scrollregion
    ...
    def confirm():
        hide_tooltip()
        selected = [path for i, path in enumerate(
            image_paths) if check_vars[i].get()]
    ...
    def update_count(*args):
        ...
        info_label.config(text=T(
            f"Vybráno: {count} z {len(image_paths)}", f"Ausgewählt: {count} von {len(image_paths)}"))
    ...
    tk.Button(bottom_frame, text=T("✓ Export s vybranými", "✓ Export mit Auswahl"), command=confirm,
              bg="#4CAF50", fg="white", width=18, font=("Segoe UI", 10, "bold")).pack(side="right", padx=5)
    tk.Button(bottom_frame, text=T("✗ Zrušit", "✗ Abbrechen"),
              command=cancel, width=12).pack(side="right")

    # Klávesové zkratky
    win.bind("<Return>", lambda e: confirm())
    win.bind("<Escape>", lambda e: cancel())

    # Při zavření okna skrýt tooltip
...
def vyber_fotky_dialog_bez_miniatur(parent, image_paths: list):
    """
    Fallback dialog bez miniatur (pokud Pillow není nainstalován).
    """
    if not image_paths:
        return []
    ...
    tk.Label(header, text=T("Vyberte fotky pro export do PDF:", "Wählen Sie Fotos für PDF-Export:"),
             font=("Segoe UI", 11, "bold")).pack(anchor="w")
    tk.Label(header, text=T(f"Celkem nalezeno: {len(image_paths)} fotek", f"Insgesamt gefunden: {len(image_paths)} Fotos"),
             fg="gray").pack(anchor="w")
    ...
    # Slovník pro uchování checkboxů a jejich stavů
    check_vars = []
    check_buttons = []

    # Vytvoření checkboxů pro každou fotku
    for i, path in enumerate(image_paths):
        var = tk.BooleanVar(value=True)
        check_vars.append(var)
...
def export_poruchy_pdf(parent, cislo: str, stroje: dict):
    """
    Export všech poruch jednoho stroje do PDF (A4 na šířku).
    parent  — okno (Toplevel), kvůli dialogům
    cislo   — číslo stroje (string nebo číslo)
    stroje  — dict se stroji (self.stroje)
    """
    import os
    ...
    try:
        ...
        messagebox.showerror(
            ...
            T("Chybí knihovna reportlab.\nNainstaluj: py -m pip install reportlab",
              "Reportlab-Bibliothek fehlt.\nInstallieren: py -m pip install reportlab"),
            parent=parent,
        )
        return
    
    # ---------- helper: obrázek na max velikost (se zachováním poměru stran) ----------
    def _img_fit(path: str, max_w, max_h):
        """Vrátí reportlab Image zmenšený tak, aby se vešel do max_w/max_h a držel poměr stran."""
        ir = ImageReader(path)
        iw, ih = ir.getSize()
        if not iw or not ih:
    ...
    if not poruchy:
        messagebox.showinfo(
            T("Export PDF", "PDF-Export"),
            T(f"Stroj {cislo} nemá žádné poruchy k exportu.",
              f"Maschine {cislo} hat keine Störungen zum Export."),
            parent=parent,
        )
        return
        ...
        try:
            ...
            header_table = Table(
                [
                    [
                        Paragraph(
                            f"<b>SGM-Wartung — {T('Poruchy stroje', 'Störungen Maschine')} {cislo}</b>",
                            title_style,
                        ),
                        img,
                    ]
            ...
            story.append(
                Paragraph(
                    f"<b>SGM-Wartung — {T('Poruchy stroje', 'Störungen Maschine')} {cislo}</b>", title_style
                )
            )
    else:
        story.append(
    ...
    story.append(
        Paragraph(
            f"{T('Výrobce', 'Hersteller')}: {vyrobce}&nbsp;&nbsp;&nbsp;&nbsp;{T('Typ', 'Typ')}: {typ_stroje}", sub_style
        )
    )
    ...
    # ---------- hlavička tabulky ----------
    header = ["ID", T("Datum", "Datum"), T("Kat", "Kat"), T("Alarm", "Alarm"), T(
        "Popis / Řešení", "Beschr. / Lösung"), T("Operátor", "Operator")]
    ...
    for p in poruchy:
        pid = str(p.get("id", ""))
        cas = p.get("cas", "") or ""
        kat = kat_ui(p.get("kategorie", ""))
        alarm = p.get("alarm", "") or ""
        popis = p.get("popis", "") or ""
        reseni = p.get("reseni", "") or ""
        op = p.get("operator_uzavrel", "") or ""

        # společný text: Popis + Řešení, každý na vlastním řádku
        lines = []
        if popis.strip():
        ...
        row = [
            Paragraph(pid, base_style),
            Paragraph(cas, base_style),
            Paragraph(kat, base_style),
            Paragraph(alarm, base_style),
            Paragraph(combo_text, base_style),
            Paragraph(op, base_style),
        ]
        data.append(row)

    # šířky sloupců (A4 landscape)
    table = Table(
        data,
        colWidths=[
            15 * mm,   # ID
            ...
            25 * mm,   # Kat
            35 * mm,   # Alarm
            110 * mm,  # Popis / Řešení — široký sloupec
            30 * mm,   # Operátor
        ],
        repeatRows=1,
    )
    ...
    # ---------- fotodokumentace stroje ----------
    from pathlib import Path

    foto_root: Path | None = None
    ...
    if foto_root is None:
        try:
            ...
            data_dir = Path("data")
        base = data_dir / "soubory" / str(cislo)
    ...
    # **Dialog pro výběr fotek s miniaturami**
    if image_paths:
        selected_paths = vyber_fotky_dialog(parent, image_paths)

        # Pokud uživatel zrušil dialog, nebudou fotky vůbec
        if selected_paths is None:
            selected_paths = []

        # Vytvoření tabulky s vybranými fotkami
        image_rows = []
                # 1 fotka = 1 stránka (velké, čitelné)
        if selected_paths:
            ...
            # dostupná plocha stránky (bez okrajů)
            max_w = doc.width
            # necháme místo na název souboru + odsazení
            ...
            for path in selected_paths:
                ...
                try:
                    ...
                    story.append(Paragraph(
                        T("Chyba načtení obrázku:", "Fehler beim Laden des Bildes:") + f" {fname_img}",
                        base_style
                    ))
...
class StrojeGrid(StrojeGrid):  # rozšíření
    ...
    def backup_zip(self):
        ...
        fname = filedialog.asksaveasfilename(
            defaultextension=".zip", initialfile=f"sgm_backup_{ts}.zip")
        if not fname:
            return
        ...
        messagebox.showinfo(T("Záloha", "Sicherung"),
                            f"{T('Uloženo', 'Gespeichert')} {fname}")

    def restore_zip(self):
        fname = filedialog.askopenfilename(filetypes=[("ZIP", "*.zip")])
        if not fname:
            return
        with zipfile.ZipFile(fname) as z:
            for name in ["stroje.csv", "poruchy.csv", "sablony_alarmu.csv"]:
                if name in z.namelist():
                    with z.open(name) as src, open(DATA_DIR/name,
                                                   "wb") as dst:
                        dst.write(src.read())
        ...
        self.nakresli_mrizku()
        messagebox.showinfo(T("Obnova", "Wiederherstellung"), T(
            "Data obnovena.", "Daten wiederhergestellt."))
